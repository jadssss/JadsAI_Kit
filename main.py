...
def main():
    ...
    parser.add_argument('--architecture', choices=['cnn', 'rnn', 'transformer', 'crnn'], default='crnn', help="Архитектура")
    parser.add_argument('--conv-layers', type=str, default="32,3,1,same;64,3,1,same;128,3,1,same", help="Conv слои: filters,kernel,stride,padding;...")
    parser.add_argument('--dense-layers', type=str, default="128", help="Dense слои (юниты, через запятую)")
    ...
    parser.add_argument('--dropout', type=float, default=0.3, help="Dropout (0-0.5)")
    parser.add_argument('--batch-norm', action='store_true', help="Включить BatchNorm")
    parser.add_argument('--loss', choices=['sparse_categorical_crossentropy', 'categorical_crossentropy', 'ctc_loss'], default='ctc_loss', help="Loss функция")
    parser.add_argument('--epochs', type=int, default=10, help="Эпохи обучения")
    parser.add_argument('--batch-size', type=int, default=64, help="Batch size")
    parser.add_argument('--load-data', type=str, help="Путь к .npz датасету")
    parser.add_argument('--captcha-dir', type=str, help="Путь к папке с [метка].png для капчи")
    ...
    parser.add_argument('--predict', type=str, help="Путь к изображению для предсказания капчи")
